...
try:
    ...
    print(e)
    print(Fore.RED + f'[ERROR] ' + Fore.RESET + 'Faild to install BenBotAsync')
    exit()

os.system('cls||clear')

intro = Fore.LIGHTGREEN_EX + """
  _____       ____   __       __              ______      ____     _    
 |_   _|    .'    '.[  |     [  |            |_   _ \   .'    '.  / |_  
   | |     |  .--.  || |.--.  | |.--.   _   __ | |_) | |  .--.  |`| |-' 
   | |   _ | |    | || '/'`\ \| '/'`\ \[ \ [  ]|  __'. | |    | | | |   
  _| |__/ ||  `--'  ||  \__/ ||  \__/ | \ '/ /_| |__) ||  `--'  | | |,  
 |________| '.____.'[__;.__.'[__;.__.'[\_:  /|_______/  '.____.'  \__/  
                                       \__.'                                      
"""
...
async def event_ready():
    os.system('cls||clear')
    print(intro)
    print(Fore.GREEN + ' [Ready!] ' + Fore.RESET + 'Client ready as ' + Fore.LIGHTGREEN_EX + f'{client.user.display_name}')
...
async def event_party_invite(invite):
    if data['joinoninvite'].lower() == 'true':
        try:
            await invite.accept()
            print(Fore.GREEN + ' [Console] ' + Fore.RESET + f'Accepted party invite from {invite.sender.display_name}')
        except Exception:
            pass
    elif data['joinoninvite'].lower() == 'false':
        if invite.sender.id in info['FullAccess']:
            await invite.accept()
            print(Fore.GREEN + ' [Console] ' + Fore.RESET + 'Accepted party invite from ' + Fore.LIGHTGREEN_EX + f'{invite.sender.display_name}')
        else:
            print(Fore.GREEN + ' [Console] ' + Fore.RESET + f'Never accepted party invite from {invite.sender.display_name}')
...
async def event_friend_request(request):
    if data['friendaccept'].lower() == 'true':
        ...
        if request.id in info['FullAccess']:
            try:
                await request.accept()
                print(Fore.GREEN + ' [Console] ' + Fore.RESET + 'Accepted friend request from ' + Fore.LIGHTGREEN_EX + f'{request.display_name}' + Fore.LIGHTBLACK_EX + f' ({lenFriends()})')
            except Exception:
                pass
        else:
    ...
    if client.user.display_name != member.display_name:
        try:
            if client.user.id in info['FullAccess']:
                print(Fore.LIGHTBLUE_EX + f' [Console] {member.display_name}' + Fore.RESET + 'has joined the lobby.')
            else:
                print(f' [Console] {member.display_name} has joined the lobby.' + Fore.LIGHTGREEN_EX + f' ({lenPartyMembers()})')
...
async def event_party_member_leave(member):
    if client.user.display_name != member.display_name:
        try:
            if client.user.id in info['FullAccess']:
                print(Fore.LIGHTRED_EX + f' [Console] {member.display_name}' + Fore.RESET + 'has left the lobby.')
            else:
                print(f' [Console] {member.display_name} has left the lobby.' + Fore.LIGHTRED_EX + f' ({lenPartyMembers()})')
        except fortnitepy.HTTPException:
            pass
